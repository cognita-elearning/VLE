...
function corolla_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  static $zebra = FALSE;
  $zebra = !$zebra;
...
//<a href="/ms-excel-2010/naprednije-mogucnosti/ispravnost-dokumenta">Ispravnost dokumenta</a>
$startIndex = strpos($link,'href=/')+10;
$offsetIndex = strpos($link,'"',strpos($link,'href=/')+10)-strpos($link,'href=/')-10;
//$A = drupal_set_message(substr($link,$startIndex,$offsetIndex));

$alias = substr($link,$startIndex,$offsetIndex);
$path = drupal_lookup_path("source", $alias);
$node = menu_get_object("node", 1, $path);
$isMandatory = $node->field_lesson_mandatory['0']['value'];
//$A = drupal_set_message($node->nid . ' ' . $link . ' ' . $isMandatory);
switch ($isMandatory) {
case 1:
	return '<li class="' . $class . ' mandatory">' . $link . $menu . "</li>\n";
break;
default:
